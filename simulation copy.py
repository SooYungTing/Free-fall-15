...
def reset_command():
    ...
    canvas.delete("all")
    canvas.create_rectangle(0, 489, 500, 700, fill='light green', outline='')
...
# Animation
frame = tk.Frame(root, background=outputFrame_bg_sky)
frame.place(x=50, y=50)
canvas = tk.Canvas(frame, width=500, height=690, bg=outputFrame_bg_sky, highlightthickness=0)
canvas.pack()
...
def start_animation():
    ...
    obj_mass = obj_mass_dict[object_var.get()] 

    obj_diamemetr_dict={"Marshmallow" : 0.05,"Brick":0.2,"Car":1.5}
    obj_diameter_animation = obj_diamemetr_dict[object_var.get()]
    ...
    obj_speed = math.sqrt((2 * (obj_mass)* ((g[planet_var.get()]))) / (0.5 * rho_air * obj_diameter_animation ** 2 * Cd))
    
    #Air resistance for each object
    if object_var.get() == "Marshmallow":
        obj_diameter_animation = 0.05
        drag_force = 0.5 * rho_air * obj_speed ** 2 * obj_diameter_animation ** 2 * Cd
        marshmallow_image = Image.open("marshmallow.png").resize((20, 20))
        marshmallow_image = marshmallow_image.rotate(ang)
        marsh = ImageTk.PhotoImage(marshmallow_image)
        object_display = canvas.create_image(x_pos, y_pos, anchor="center", image=marsh)
        y_pos_limit = 483
        
    elif object_var.get() == "Brick":
        obj_diameter_animation = 0.2
        drag_force = 0.5 * rho_air * obj_speed ** 2 * obj_diameter_animation ** 2 * Cd
        brick_image = Image.open("brick.png").resize((80, 80))
        brick_image = brick_image.rotate(ang)
        brick = ImageTk.PhotoImage(brick_image)
        object_display = canvas.create_image(x_pos, y_pos, anchor="center", image=brick)
        if ang == 90 or ang ==270:
            y_pos_limit = 455
        else:
            y_pos_limit = 478

    elif object_var.get() == "Car":
        obj_diameter_animation = 1.5
        drag_force = 0.5 * rho_air * obj_speed ** 2 * obj_diameter_animation ** 2 * Cd
        car_image = Image.open("car.png").resize((200,200))
        car_image = car_image.rotate(ang)
        car = ImageTk.PhotoImage(car_image)
        object_display = canvas.create_image(x_pos, y_pos, anchor='center',image=car)
        if ang == 90 or ang == 270:
            y_pos_limit = 439
        elif ang == 180:
            y_pos_limit = 450
    ...
    while True:
        canvas.move(object_display, x_vel, y_vel)
        canvas.update()
        time.sleep(0.03)
        get_coords = canvas.coords(object_display)
        print(get_coords)
        x_pos, y_pos = get_coords
        acceleration = g[planet_var.get()]
        y_vel -= -drag_force*dt
...
# Disable Air Resistance Button
no_air_res_button = tk.Button(inputFrame, text="Disable Air Resistance", font=("Times New Roman", 18),command=disable_air_resistance_command)
no_air_res_button.place(relx=0.22, rely=0.66, relwidth=0.15)
