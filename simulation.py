...
class Simulation(tk.Toplevel):
    def __init__(self) -> None:
        ...
        self.outputFrame_land.place(x=0, y=screen_height // 1.6, width=screen_width // 10 + screen_width // 2,
                                    height=screen_height)

        self.inputText = tk.Label(self.inputFrame, text="Input", font=("Times New Roman", "23", "bold"),
                                  bg=self.inputFrame_bg, fg=self.textBg)
        self.inputText.place(relx=0.185, rely=0.010)

        self.outputText = tk.Label(self.outputFrame_sky, text="Output", font=("Times New Roman", "23", "bold"),
                                   bg=self.outputFrame_bg_sky, fg=self.textBg)
        ...
        # temperature scale
        self.temp_slider = tk.Scale(self.inputFrame, from_=20, to=-20, orient='vertical', resolution=0.5,
                                    sliderlength=50, length=400, width=50, bg=self.inputFrame_bg, bd=2,
                                    highlightthickness=0, highlightcolor="#d7d7d7", fg=self.textBg)
        self.temp_slider.set(0.0)
        self.temp_label = tk.Label(self.inputFrame, text="Temperature (°C)", font=("Times New Roman", "14"),
                                   bg=self.inputFrame_bg, fg=self.textBg)
        ...
        # height scale
        self.height_slider = tk.Scale(self.inputFrame, from_=1000, to=100, orient='vertical', resolution=1,
                                      sliderlength=50, length=400, width=50, bg=self.inputFrame_bg, bd=2,
                                      highlightthickness=0, highlightcolor="#d7d7d7", fg=self.textBg)
        self.height_slider.set(550)
        self.height_label = tk.Label(self.inputFrame, text="Height (m)", font=("Times New Roman", "14"),
                                     bg=self.inputFrame_bg, fg=self.textBg)
        ...
        self.planet_menu = tk.OptionMenu(self.inputFrame, self.planet_var, "Earth | 9.81 m/s^2", "Venus | 8.87 m/s^2",
                                         "Mercury | 3.7 m/s^2", "Mars | 3.71 m/s^2", "Moon | 1.62 m/s^2",
                                         command=lambda option: self.changeBackground(self.planet_var.get()))
        ...
        all_input_buttons = [
            # Rotation button
            {'text': 'Rotate 90°', 'font': ('Times new Roman', 18), 'command': self.rotation_command, 'relx': 0.22,
             'rely': 0.56, 'relwidth': 0.15},
            # Disable air resistance button
            {'text': 'Disable Air Resistance', 'font': ('Times new Roman', 18),
             'command': self.disable_air_resistance_command, 'relx': 0.22, 'rely': 0.66, 'relwidth': 0.15},
            # Instruction guide button
            {'text': 'Instruction Guide', 'font': ('Times new Roman', 18), 'command': self.open_instruction_page,
             'relx': 0.22, 'rely': 0.76, 'relwidth': 0.15},
            # Reset button
            {'text': 'Reset', 'font': ('Times new Roman', 20), 'command': self.reset_command, 'relx': 0.02,
             'rely': 0.94, 'relwidth': 0.08},
            # Back button
            {'text': 'Back', 'font': ('Times new Roman', 20), 'command': self.back_command, 'relx': 0.21, 'rely': 0.94,
             'relwidth': 0.08},
            # Start button
            {'text': 'Start', 'font': ('Times new Roman', 20), 'command': self.start_command, 'relx': 0.3, 'rely': 0.94,
             'relwidth': 0.08},
        ]

        for properties in all_input_buttons:
            button = tk.Button(self.inputFrame, text=properties['text'], font=properties['font'],
        ...
        self.initial_velocity_text = tk.Label(self.initial_velocity_frame, text="Velocity:",
                                              font=("Times New Roman", 18), bg=self.outputFrame_bg_land, fg=self.textBg)
        self.initial_velocity_value_frame = tk.Frame(self.initial_velocity_frame, highlightbackground="#B0A3F5",
                                                     highlightthickness=2, bg="gray", width=100, height=25)
        self.initial_velocity_value = tk.Label(self.initial_velocity_value_frame, text="0.00",
                                               font=("Times New Roman", 18))
        self.initial_velocity_unit = tk.Label(self.initial_velocity_frame, text=" m/s", font=("Times New Roman", 18),
                                              bg=self.outputFrame_bg_land, fg=self.textBg)
        ...
        self.ke_text = tk.Label(self.ke_frame, text="Kinetic Energy:", font=("Times New Roman", 18),
                                bg=self.outputFrame_bg_land, fg=self.textBg)
        self.ke_value_frame = tk.Frame(self.ke_frame, highlightbackground="#B0A3F5", highlightthickness=2, bg="gray",
                                       width=100, height=25)
        self.ke_value = tk.Label(self.ke_value_frame, text="0.00", font=("Times New Roman", 18))
        self.ke_unit = tk.Label(self.ke_frame, text=" J", font=("Times New Roman", 18), bg=self.outputFrame_bg_land,
                                fg=self.textBg)
        ...
        self.impact_text = tk.Label(self.impact_force_frame, text="Impact Force:", font=("Times New Roman", 18),
                                    bg=self.outputFrame_bg_land, fg=self.textBg)
        self.impactforce_value_frame = tk.Frame(self.impact_force_frame, highlightbackground="#B0A3F5",
                                                highlightthickness=2, bg="gray", width=100, height=25)
        self.impact_force_value = tk.Label(self.impactforce_value_frame, text="0.00", font=("Times New Roman", 18))
        self.impact_unit = tk.Label(self.impact_force_frame, text=" N", font=("Times New Roman", 18),
                                    bg=self.outputFrame_bg_land, fg=self.textBg)
        ...
        self.gpe_text = tk.Label(self.gpe_frame, text="G.P.E:", font=("Times New Roman", 18),
                                 bg=self.outputFrame_bg_land, fg=self.textBg)
        self.gpe_value_frame = tk.Frame(self.gpe_frame, highlightbackground="#B0A3F5", highlightthickness=2, bg="gray",
                                        width=100, height=25)
        self.gpe_value = tk.Label(self.gpe_value_frame, text="0.00", font=("Times New Roman", 18))
        self.gpe_unit = tk.Label(self.gpe_frame, text=" J", font=("Times New Roman", 18), bg=self.outputFrame_bg_land,
                                 fg=self.textBg)
        ...
        self.acc_text = tk.Label(self.acceleration_frame, text="Acceleration:", font=("Times New Roman", 18),
                                 bg=self.outputFrame_bg_land, fg=self.textBg)
        self.acc_value_frame = tk.Frame(self.acceleration_frame, highlightbackground="#B0A3F5", highlightthickness=2,
                                        bg="gray", width=100, height=25)
        self.acc_value = tk.Label(self.acc_value_frame, text="0.00", font=("Times New Roman", 18))
        self.acc_unit = tk.Label(self.acceleration_frame, text=" m/s^2", font=("Times New Roman", 18),
                                 bg=self.outputFrame_bg_land, fg=self.textBg)
    ...
    def changeBackground(self, selected_value):
        ...
        match selected_value:
            ...
            case 'Venus | 8.87 m/s^2':
                self.inputFrame_bg = '#ddd8d4'
                self.outputFrame_bg_sky = '#8b91a1'
                self.outputFrame_bg_land = '#bbb7ab'
                self.textBg = '#000000'
                self.updateColours()

            case "Mars | 3.71 m/s^2":
                self.inputFrame_bg = '#c67b5c'
                self.outputFrame_bg_sky = '#ff9d6f'
                self.outputFrame_bg_land = '#e27b58'
                self.textBg = '#000000'
                self.updateColours()
    ...
    def updateColours(self):
        ...
        # Input widgets
        self.temp_slider.config(bg=self.inputFrame_bg, fg=self.textBg);
        self.temp_label.config(bg=self.inputFrame_bg, fg=self.textBg)
        self.height_slider.config(bg=self.inputFrame_bg, fg=self.textBg);
        self.height_label.config(bg=self.inputFrame_bg, fg=self.textBg)

        # Output widgets
        self.acc_text.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.acc_unit.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.acceleration_frame.config(bg=self.outputFrame_bg_land)
        self.impact_text.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.impact_unit.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.impact_force_frame.config(bg=self.outputFrame_bg_land)
        self.initial_velocity_text.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.initial_velocity_unit.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.initial_velocity_frame.config(bg=self.outputFrame_bg_land)
        self.gpe_text.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.gpe_unit.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.gpe_frame.config(bg=self.outputFrame_bg_land)
        self.ke_text.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.ke_unit.config(fg=self.textBg, bg=self.outputFrame_bg_land);
        self.ke_frame.config(bg=self.outputFrame_bg_land)
        self.frame.config(bg=self.outputFrame_bg_sky);
        self.canvas.config(bg=self.outputFrame_bg_sky);
        self.canvas.itemconfig(self.rectangle, fill=self.outputFrame_bg_land)
    ...
    def start_animation(self):
        ...
        obj_speed = math.sqrt((2 * (obj_mass) * ((self.g[self.planet_var.get()]))) / (
                    0.5 * self.rho_air * obj_diameter_animation ** 2 * self.Cd))
        ...
        while True:
            ...
            y_vel = min(y_vel, y_pos_limit - y_pos)
